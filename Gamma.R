...
parameters_gamma <- read_excel("C:/Results/parameters.xlsx",sheet="Gamma")
attach(parameters_gamma) ; dist <- "Gamma[1:5,]"

# Function to obtain length run for a simulated gamma control chart   
...
# Function to obtain a summary for a length run of a 
...
resfinal_gamma    <- apply(parameters_gamma[1:5,],1, results_gamma, 
                           n="n", phi="phi", lambda="lambda", L="L" , k="k",
                           Shape="Shape" , Scale="Scale")
file.name         <- paste("C:/Resultados/" , dist,".csv")
